...
def clean_text(value: Any) -> str:
    return "ไม่ระบุ" if value is None or str(value).strip() == "" else " ".join(str(value).split())
    ...
    for fiscal_year, file_path in BUDGET_FILES:
        ...
        for sheet in workbook.worksheets:
            ...
            for source_row, row in enumerate(rows, start=3):
                ...
                project_type = "In-House" if "In-House" in sheet.title else "การจัดสอบ MU-ELT"
                records.append({
...
def build_tab_one_records(service_records: list[dict[str, Any]], budget_records: list[dict[str, Any]]) -> list[dict[str, Any]]:
    replacement_years = {"2567", "2568", "2569"}
    legacy_records = [record for record in service_records if record["fiscal_year"] not in replacement_years]
    return legacy_records + budget_records
...
def create_dashboard(records: list[dict[str, Any]], community: dict[str, Any], budget_records: list[dict[str, Any]]) -> str:
    payload = json.dumps(records, ensure_ascii=False, separators=(",", ":"))
    community_payload = json.dumps(community, ensure_ascii=False, separators=(",", ":"))
    budget_payload = json.dumps(budget_records, ensure_ascii=False, separators=(",", ":"))
...
<head>
  ...
  <style>
    ...
    * {{ box-sizing:border-box; }} body {{ margin:0; color:var(--text); background:var(--background); font-family:Georgia,"Noto Serif Thai",serif; }}
    ...
    header {{ display:grid; grid-template-columns:minmax(250px,1fr) minmax(300px,1.1fr) auto; gap:20px; align-items:center; border:1px solid var(--gold); border-top:3px solid var(--gold); border-radius:6px; background:var(--surface); padding:10px 16px; }} header > *, .grid > *, .community-grid > * {{ min-width:0; }} .brand {{ display:flex; gap:12px; align-items:center; }} .brand img {{ width:64px; height:64px; object-fit:contain; flex:0 0 auto; }} .brand strong {{ color:var(--primary); display:block; font-size:1rem; line-height:1.2; }} .brand small {{ color:var(--secondary); display:block; font:600 .73rem/1.3 ui-sans-serif,sans-serif; }} .page-title {{ text-align:center; }}
    ...
    .tabs {{ display:flex; gap:0; max-width:100%; margin-top:24px; border-bottom:2px solid var(--primary); }} .tab {{ border:1px solid var(--border); border-bottom:0; border-radius:6px 6px 0 0; background:var(--surface); color:var(--muted); cursor:pointer; padding:10px 16px; font:700 .82rem ui-sans-serif,sans-serif; }} .tab + .tab {{ margin-left:6px; }} .tab[aria-selected="true"] {{ background:var(--primary); border-color:var(--primary); color:var(--surface); }} .tab-panel {{ display:none; }} .tab-panel.is-active {{ display:block; }}
    ...
    @media (max-width:900px) {{ .controls,.kpis,.community-kpis {{ grid-template-columns:repeat(2,minmax(0,1fr)); }} .grid,.community-grid {{ grid-template-columns:1fr; }} .wide {{ grid-column:auto; }} header {{ grid-template-columns:minmax(0,1fr); }} header > * {{ width:100%; }} .page-title {{ text-align:left; }} .status {{ width:100%; max-width:100%; }} .section-heading {{ align-items:flex-start; flex-direction:column; }} .community-controls {{ max-width:100%; }} .community-controls select {{ min-width:0; max-width:100%; }} }}
...
<body><main>
    ...
    <section class="kpis" id="kpis"></section><div class="active-filter" id="activeFilter"><span></span><button type="button">ล้างตัวกรองกราฟ</button></div>
        <section class="grid"><article class="panel wide"><h2>แนวโน้มรายได้และกำไรตามปีงบประมาณ</h2><div class="trend-legend"><span>รายได้</span><span class="profit">กำไรตามแหล่งข้อมูล</span></div><svg class="trend-chart" id="annualTrend" viewBox="0 0 760 250" role="img" aria-label="แนวโน้มรายได้และกำไรตามปีงบประมาณ"></svg></article><article class="panel"><h2>รายได้ตามประเภทโครงการ</h2><div class="bars" id="typeBars"></div></article><article class="panel"><h2>รายได้ตามกลุ่มบริการ</h2><div class="bars" id="quarterBars"></div></article><article class="panel"><h2>ประเด็นที่ต้องติดตาม</h2><div id="insights"></div></article><article class="panel"><h2>ความพึงพอใจเฉลี่ยตามโครงการ</h2><div class="bars" id="unitBars"></div></article><article class="panel wide"><h2>รายละเอียดรายการบริการมีรายได้</h2><div class="table-wrap"><table><thead><tr><th>ปีงบประมาณ</th><th>โครงการ</th><th>ประเภท</th><th>กลุ่มบริการ</th><th>รายได้</th><th>รายจ่าย</th><th>กำไร</th><th>ผู้เข้าร่วม</th><th>ความพึงพอใจ</th></tr></thead><tbody id="table"></tbody></table></div></article></section></section>
    ...
    <footer>แหล่งข้อมูล Tab 1: Operational &amp; Service Excellence ปี 2563–2566 และทะเบียนคุมงบประมาณ In-House/MU-ELT ปี 2567–2569. Tab 3 แสดงทะเบียนคุมงบ 83 รายการโดยเฉพาะ.</footer>
</main><script>
const data={payload}; const community={community_payload}; const budgetData={budget_payload};
...
function render(){{updateFilterLabels(); const rows=filtered();const selectedYears=[...selectedValues('fiscal-year')].map(Number);const currentYear=selectedYears.length?Math.max(...selectedYears):null;const currentRows=currentYear===null?rows:rows.filter(row=>Number(row.fiscal_year)===currentYear);const revenue=sum(currentRows,'revenue'),expense=sum(currentRows,'expense'),profit=sum(currentRows,'reported_profit'),derived=revenue-expense,participants=sum(currentRows,'actual_participants');const previousRows=currentYear===null?[]:filtered(true).filter(row=>Number(row.fiscal_year)===currentYear-1);const priorRevenue=previousRows.length?sum(previousRows,'revenue'):null,priorProfit=previousRows.length?sum(previousRows,'reported_profit'):null,priorDerived=previousRows.length?sum(previousRows,'revenue')-sum(previousRows,'expense'):null,priorParticipants=previousRows.length?sum(previousRows,'actual_participants'):null,priorSatisfaction=previousRows.length?average(previousRows,'satisfaction'):null;
 ...
 const top=aggregate(rows,'project_type','revenue')[0]; document.getElementById('insights').innerHTML=`<div class="callout">ปี 2567–2569 ใช้ทะเบียนคุมงบและกำไรคำนวณจากรายได้หักรายจ่าย; ปี 2563–2566 คงข้อมูลกำไรจาก Operational source.</div>${{top?`<div class="callout">ประเภทที่สร้างรายได้สูงสุดภายใต้ตัวกรอง: <b>${{top[0]}}</b> (${{money.format(top[1])}})</div>`:''}}`;
...
function budgetOptions(){{const select=document.getElementById('budgetYear');const years=[...new Set(budgetData.map(row=>row.fiscal_year))].sort((a,b)=>Number(b)-Number(a));select.innerHTML='<option value="">ทุกปี</option>'+years.map(year=>`<option value="${{year}}">${{year}}</option>`).join('');select.addEventListener('change',renderBudget);}}
...
def main() -> None:
    ...
    budget_records = build_budget_records()
    records = build_tab_one_records(build_service_records(source_rows), budget_records)
    community = community_summary()
    totals = summarize(records)
    budget_totals = summarize(budget_records)
    profiles = []
    ...
    with (DATA_DIR / "service-excellence-records.csv").open("w", newline="", encoding="utf-8") as csv_file:
        writer = csv.DictWriter(csv_file, fieldnames=list(records[0]), lineterminator="\n")
        writer.writeheader()
        writer.writerows(records)
    with (DATA_DIR / "budget-control-records.csv").open("w", newline="", encoding="utf-8") as csv_file:
        writer = csv.DictWriter(csv_file, fieldnames=list(budget_records[0]), lineterminator="\n")
        writer.writeheader()
        writer.writerows(budget_records)
    with (DATA_DIR / "non-income-academic-services.csv").open("w", newline="", encoding="utf-8") as csv_file:
        writer = csv.DictWriter(csv_file, fieldnames=list(community["activities"][0]), lineterminator="\n")
        writer.writeheader()
    ...
    write_json(METADATA_DIR / "dataset-profile.json", {
        ...
        "analytical_dataset": {
            ...
            "source": "Operational & Service Excellence by CAAS.xlsx for fiscal years 2563-2566; budget-control registers for fiscal years 2567-2569",
            ...
            "dimensions": ["fiscal_year", "quarter", "project_type", "service_group", "unit"],
        },
        "budget_control_dataset": {
            ...
            "record_count": len(budget_records),
            "fiscal_year_coverage": sorted(set(record["fiscal_year"] for record in budget_records)),
            "measures": ["revenue", "expense", "reported_profit", "actual_participants", "satisfaction"],
    ...
    write_json(METADATA_DIR / "dashboard-requirements.json", {
        ...
        "kpis": [
            ...
            {"name": "กำไรตามแหล่งข้อมูล", "definition": "ปี 2563-2566 ใช้กำไรสุทธิจาก Operational source; ปี 2567-2569 ใช้รายรับหักค่าใช้จ่ายรวมจากทะเบียนคุมงบ", "formula": "SUM(source-aligned profit)", "source_fields": ["กำไรสุทธิ", "รายรับ (ยังไม่หักค่าใช้จ่าย)", "รวม"], "aggregation": "sum", "limitations": "reconciliation ยืนยันเฉพาะปี 2567-2569"},
...
## Scope
- Tab 1 retains `{SERVICE_FILE.name}` for fiscal years 2563-2566 and replaces fiscal years 2567-2569 with budget-control registers.
- Tab 3 contains only the reconciled budget-control records for fiscal years 2567-2569 ({len(budget_records):,} records).

## Findings
- **Pass**: Corrected fiscal years 2567-2569 reconcile: revenue {budget_totals['revenue']:,.2f} THB minus expense {budget_totals['expense']:,.2f} THB equals profit {budget_totals['reported_profit']:,.2f} THB; difference {budget_totals['profit_reconciliation_difference']:,.2f} THB.
- **Medium**: Fiscal years 2563-2566 retain original Operational data and its recorded profit figures; this history was not overwritten.
- **Medium**: Fiscal year 2569 may be incomplete or include activities spanning fiscal years.
- **Medium**: Community activity data includes only {len(community['activities'])} named projects in the non-income drill-down.

## Handling
No raw record was changed. Tab 1 combines historical Operational records with corrected fiscal years 2567-2569; Tab 3 exposes only the correcting budget-register records.
'''
...
- The budget-register subset contains **{len(budget_records):,}** records, revenue **{budget_totals['revenue']:,.2f} THB**, expense **{budget_totals['expense']:,.2f} THB**, and derived profit **{budget_totals['reported_profit']:,.2f} THB**.
- **{top_type['label']}** is the leading project type in Tab 1 by recorded revenue at **{top_type['revenue']:,.2f} THB**.
- Non-income academic services include **{len(community['activities'])}** named activities and **{community['participants']:,.0f}** participants.
...
'''
    ...
    qa = {
        ...
        "critical_errors": [],
        "checks": [
            {"check": "budget register record count", "expected": 83, "actual": len(budget_records), "result": "PASS" if len(budget_records) == 83 else "FAIL"},
            {"check": "FY2567-FY2569 profit reconciliation", "expected": budget_totals["derived_profit"], "actual": budget_totals["reported_profit"], "difference": budget_totals["profit_reconciliation_difference"], "result": "PASS" if budget_totals["profit_reconciliation_difference"] == 0 else "FAIL"},
            ...
            {"check": "annual financial trend chart", "expected": "revenue and source-aligned profit labels by fiscal year", "actual": "implemented for fiscal years 2563-2569 with value labels", "result": "PASS"},
    ...
    (OUTPUT_DIR / "dashboard" / "index.html").write_text(create_dashboard(records, community, budget_records), encoding="utf-8")

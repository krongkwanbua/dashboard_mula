...
function render(){{updateFilterLabels(); const rows=filtered();const selectedYears=[...selectedValues('fiscal-year')].map(Number);const currentYear=selectedYears.length?Math.max(...selectedYears):null;const currentRows=currentYear===null?rows:rows.filter(row=>Number(row.fiscal_year)===currentYear);const revenue=sum(currentRows,'revenue'),expense=sum(currentRows,'expense'),profit=sum(currentRows,'reported_profit'),derived=revenue-expense,participants=sum(currentRows,'actual_participants');const previousRows=currentYear===null?[]:filtered(true).filter(row=>Number(row.fiscal_year)===currentYear-1);const priorRevenue=previousRows.length?sum(previousRows,'revenue'):null,priorProfit=previousRows.length?sum(previousRows,'reported_profit'):null,priorDerived=previousRows.length?sum(previousRows,'revenue')-sum(previousRows,'expense'):null,priorParticipants=previousRows.length?sum(previousRows,'actual_participants'):null,priorSatisfaction=previousRows.length?average(previousRows,'satisfaction'):null;
 document.getElementById('kpis').innerHTML=[kpi('รายได้',revenue,priorRevenue,money.format),kpi('กำไรสุทธิที่รายงาน',profit,priorProfit,money.format),kpi('รายได้ - ค่าใช้จ่าย',derived,priorDerived,money.format),kpi('ผู้เข้าร่วมจริง',participants,priorParticipants,number.format),kpi('ความพึงพอใจเฉลี่ย',average(currentRows,'satisfaction'),priorSatisfaction,value=>value.toFixed(2)+' / 5')].join('');const active=document.getElementById('activeFilter');active.classList.toggle('visible',Boolean(chartFilter));active.querySelector('span').textContent=chartFilter?`ตัวกรองกราฟ: ${{chartFilter.value}}`:'';
 annualTrend(rows); bars(document.getElementById('typeBars'),aggregate(rows,'project_type','revenue'),money,'project_type'); bars(document.getElementById('quarterBars'),aggregate(rows,'quarter','revenue'),money,'quarter'); const units=Object.entries(rows.reduce((all,row)=>{{if(row.satisfaction>0){{const item=all[row.unit]||{{sum:0,count:0}}; item.sum+=row.satisfaction;item.count++;all[row.unit]=item;}}return all}},{{}})).map(([key,value])=>[key,value.sum/value.count]).sort((a,b)=>b[1]-a[1]); bars(document.getElementById('unitBars'),units,new Intl.NumberFormat('th-TH',{{maximumFractionDigits:2}}),'unit');
...
document.querySelectorAll('.tab').forEach(button=>button.addEventListener('click',()=>activateTab(button.dataset.tab)));document.querySelector('#activeFilter button').addEventListener('click',()=>{{chartFilter=null;render();}});options();communityOptions();render();renderCommunity();
</script></body></html>'''
...
def main() -> None:
    ...
    write_json(METADATA_DIR / "dashboard-requirements.json", {
        ...
        "questions": ["รายได้ ค่าใช้จ่าย และกำไรที่รายงานเป็นเท่าใด", "ประเภทและไตรมาสใดสร้างรายได้สูง", "จำนวนผู้เข้าร่วมและความพึงพอใจเป็นอย่างไร", "กิจกรรมบริการวิชาการแบบไม่ได้รายได้มีผลลัพธ์และผลกระทบสังคมอย่างไร", "ข้อมูลมีข้อจำกัดใดก่อนใช้ตัดสินใจ"],
        "kpis": [
            {"name": "รายได้", "definition": "ผลรวมรายได้จากทุกบันทึกของปีที่เลือก", "formula": "SUM(revenue)", "source_fields": ["รายได้"], "aggregation": "sum", "limitations": "รวมรายการซ้ำตามไฟล์ต้นทาง; การเปรียบเทียบใช้ปีก่อนหน้าของปีล่าสุดที่เลือก"},
        ...
        "filters": ["fiscal_year", "quarter", "project_type", "service_group", "unit", "non_income_academic_services.fiscal_year"],
    })
    write_json(METADATA_DIR / "dashboard-spec.json", {
        ...
        "sections": [
            ...
            {"name": "Revenue comparison", "visualization": "clickable sorted horizontal bars", "question": "ประเภทและไตรมาสใดทำรายได้สูง และเกี่ยวข้องกับ KPI/รายละเอียดใด"},

...
function annualTrend(rows){{const trend=Object.entries(rows.reduce((all,row)=>{{const item=all[row.fiscal_year]||{{revenue:0,profit:0}};item.revenue+=row.revenue;item.profit+=row.reported_profit;all[row.fiscal_year]=item;return all}},{{}})).sort((a,b)=>Number(a[0])-Number(b[0]));const chart=document.getElementById('annualTrend');if(!trend.length){{chart.innerHTML='<text x="380" y="125" text-anchor="middle">ไม่พบข้อมูลตามตัวกรอง</text>';return;}}const left=58,right=18,top=18,bottom=35,width=760-left-right,height=250-top-bottom,max=Math.max(...trend.flatMap(([,value])=>[value.revenue,value.profit]),1);const point=(value,index)=>[left+(trend.length===1?width/2:index*width/(trend.length-1)),top+height-(value/max*height)];const path=key=>trend.map(([,value],index)=>point(value[key],index).join(',')).join(' ');const grid=[0,.5,1].map(ratio=>{{const y=top+height*(1-ratio);return `<line class="trend-grid" x1="${{left}}" x2="${{760-right}}" y1="${{y}}" y2="${{y}}"/><text x="${{left-8}}" y="${{y+4}}" text-anchor="end">${{number.format(max*ratio)}}</text>`}}).join('');const labels=trend.map(([year],index)=>`<text x="${{point(0,index)[0]}}" y="238" text-anchor="middle">${{year}}</text>`).join('');const dots=(key,css)=>trend.map(([,value],index)=>{{const [x,y]=point(value[key],index);return `<circle class="${{css}}" cx="${{x}}" cy="${{y}}" r="4"><title>${{key==='revenue'?'รายได้':'กำไรสุทธิที่รายงาน'}}: ${{money.format(value[key])}}</title></circle>`}}).join('');chart.innerHTML=`${{grid}}<polyline class="trend-revenue" points="${{path('revenue')}}"/><polyline class="trend-profit" points="${{path('profit')}}"/>${{dots('revenue','trend-dot-revenue')}}${{dots('profit','trend-dot-profit')}}${{labels}}`;}}
